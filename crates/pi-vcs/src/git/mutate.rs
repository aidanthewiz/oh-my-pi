...
impl GitRepo {
	...
	pub async fn commit_create(
		...
		self
			.head_sha()?
			.ok_or_else(|| Error::backend("git commit", "commit completed without HEAD"))
	}
...
mod tests {
	...
	#[tokio::test]
	async fn mutate_stage_commit_amend_and_empty() {
		...
		assert!(
			repo
				.commit_create("empty", &CommitOptions::default(), None)
				.await
				.is_err()
		);
		...
		repo.stage_files(&["a".into()]).unwrap();
		let amended = repo
			.commit_create("amended", &CommitOptions { amend: true, ..Default::default() }, None)
			.await
			.unwrap();
	...
	#[tokio::test]
	async fn stage_commit_survives_unadvanced_index_mtime() {
		...
		let sha = repo
			.commit_create("change", &CommitOptions::default(), None)
			.await
			.unwrap();
	...
	#[cfg(unix)]
	#[tokio::test]
	async fn commit_hooks_match_git_commit_behavior() {
		let (temp, repo) = fixture();
		...
		let error = repo
			.commit_create("blocked", &CommitOptions::default(), None)
			.await
			.unwrap_err();
		assert!(matches!(
			&error,
		...
		repo
			.commit_create("original", &CommitOptions::default(), None)
			.await
			.unwrap();
		assert_eq!(
		...
		repo
			.commit_create("missing hook is skipped", &CommitOptions::default(), None)
			.await
			.unwrap();
		...
		repo
			.commit_create("non-executable hook is skipped", &CommitOptions::default(), None)
			.await
			.unwrap();
		fs::remove_file(&pre_commit).unwrap();
		repo
	...
	#[cfg(unix)]
	#[tokio::test]
	async fn commit_uses_only_the_supplied_hook_environment() {
		let (temp, repo) = fixture();
		let hook = temp.path().join(".git/hooks/pre-commit");
		write_hook(
			&hook,
			"printf '%s|%s' \"${HOME-unset}\" \"${SAFE_MARKER-unset}\" > .git/hook-env",
			true,
		);
		fs::write(temp.path().join("a"), "filtered\n").unwrap();
		repo.stage_files(&["a".into()]).unwrap();
		let env = BTreeMap::from([
			("PATH".to_owned(), std::env::var("PATH").unwrap_or_default()),
			("SAFE_MARKER".to_owned(), "safe".to_owned()),
		]);
		repo
			.commit_create(
				"filtered environment",
				&CommitOptions { env: Some(env), ..CommitOptions::default() },
				None,
			)
			.await
			.unwrap();
		assert_eq!(fs::read_to_string(temp.path().join(".git/hook-env")).unwrap(), "unset|safe");
	}

	#[cfg(unix)]
	#[tokio::test]
	async fn commit_honors_repository_signing_config() {
		let (temp, repo) = fixture();
		let signer = temp.path().join("reject-signer");
		write_hook(&signer, "echo 'test signer invoked' >&2\nexit 42", true);
		git(temp.path(), &["config", "commit.gpgSign", "true"]);
		git(temp.path(), &["config", "gpg.format", "openpgp"]);
		git(temp.path(), &["config", "user.signingKey", "test-key"]);
		git(temp.path(), &["config", "gpg.program", signer.to_str().unwrap()]);
		fs::write(temp.path().join("a"), "signed\n").unwrap();
		repo.stage_files(&["a".into()]).unwrap();
		let before = git(temp.path(), &["rev-parse", "HEAD"]);
		let error = repo
			.commit_create("signed commit", &CommitOptions::default(), None)
			.await
			.unwrap_err();
		assert!(matches!(
			&error,
			Error::Cli { stderr, .. } if stderr.contains("test signer invoked")
		));
		assert_eq!(git(temp.path(), &["rev-parse", "HEAD"]), before);
	}

	#[tokio::test]
	async fn mutate_checkout_branches_and_resets() {
		let (temp, repo) = fixture();
		repo.create_branch("other", "HEAD", false).unwrap();
		fs::write(temp.path().join("a"), "main\n").unwrap();
		repo.stage_files(&["a".into()]).unwrap();
		let main = repo
			.commit_create("main", &CommitOptions::default(), None)
			.await
			.unwrap();
	...
	#[tokio::test]
	async fn mutate_clean_and_alternate_index() {
		let (temp, repo) = fixture();
		fs::write(temp.path().join(".gitignore"), "ignored\n").unwrap();
		repo.stage_files(&[".gitignore".into()]).unwrap();
		repo
			.commit_create("ignore", &CommitOptions::default(), None)
			.await
			.unwrap();
	...
	#[tokio::test]
	async fn clean_respects_nested_ignores_and_preserves_submodules() {
		let (temp, repo) = fixture();
		...
		repo
			.commit_create("add nested gitignore", &CommitOptions::default(), None)
			.await
			.unwrap();
