...
impl From<VcsCommitOptions> for core::CommitOptions {
	fn from(v: VcsCommitOptions) -> Self {
		Self {
			...
			files:       v.files.unwrap_or_default(),
			env:         v.env.map(|values| values.into_iter().collect()),
		}
	}
}
...
impl VcsGitRepo {
	...
	pub fn commit_create<'e>(
		...
		vcs_future(env, "vcs.commitCreate", async move {
			repo.commit_create(&message, &options, cancel).await
		})
	}
		...
		vcs_future(env, "vcs.fetch", async move {
			repo
				.fetch(
					...
					cancel,
					environment.map(|values| values.into_iter().collect()),
				)
				.await
		})
	}
