...
#[inline]
fn starts_flow_arrow(text: &str) -> bool {
	["-->", "-.->", "---", "-.-"]
		.iter()
...
fn parse_node_ref(text: &str) -> Option<ParsedNodeRef<'_>> {
	let id_end = text
		.char_indices()
		.find_map(|(index, c)| {
			(!is_id_char(c) || (index > 0 && starts_flow_arrow(&text[index..]))).then_some(index)
		})
		.unwrap_or(text.len());
	if id_end == 0 {
		return None;
...
mod tests {
	...
	#[test]
	fn parses_compact_edges_after_hyphenated_ids() {
		let graph = parse_flowchart("graph TD\nStart-->Stop\nfoo-bar-.->baz-qux").unwrap();
		assert_eq!(
			graph
				.edges
				.iter()
				.map(|edge| (edge.source.as_str(), edge.target.as_str(), edge.style))
				.collect::<Vec<_>>(),
			[("Start", "Stop", EdgeStyle::Solid), ("foo-bar", "baz-qux", EdgeStyle::Dotted)],
		);
	}
	#[test]
