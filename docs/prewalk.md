...
```bash
coreforge config set prewalk.enabled true
```
...
```bash
coreforge --prewalk
coreforge --prewalk-into @smol
coreforge --prewalk-into openai/gpt-5-mini
```

At startup, Coreforge resolves the target with the normal model-role and model-matching rules. If the target cannot be resolved or has no configured credentials, Coreforge prints a warning and starts with prewalk unarmed.

## Handoff trigger

An armed prewalk injects a planning nudge. When the `todo` tool is active, any successful `todo` call—including the read-only `view` operation—opens the handoff gate. Coreforge then switches models after the first completed `edit` or `write` call.
...
Run the slash command to arm prewalk without restarting Coreforge or enabling it in config:
