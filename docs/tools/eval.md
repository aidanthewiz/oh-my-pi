...
JavaScript dependencies install with `%bun add csv-parse` into a Coreforge-managed package environment shared by sessions in the same project; each worker keeps its own variables. Installation does not restart the worker; already imported modules stay cached until an explicit reset. Lifecycle scripts are disabled; packages needing native builds/postinstall must be prepared explicitly. `%environment project` selects the repository itself for package/lockfile changes; `%environment managed` returns to Coreforge-managed dependencies. The selection persists for later calls through that eval tool.
...
- JS: `await agent(prompt, { agent?, label?, schema?, schemaMode?, contextFilePolicy?, isolated?, apply?, merge?, tools? })`; Python uses keyword arguments (`schema_mode`, `context_file_policy`).
...
- Parent context files are inherited by default. Set `contextFilePolicy="none"` or `context_file_policy="none"` for a context-agnostic child.
