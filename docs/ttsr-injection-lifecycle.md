...
`coreforge ttsr list` shows each rule's `question`. `coreforge ttsr test` never calls the judge, so question rules report as not triggered, with their question shown. `coreforge ttsr scan` skips question rules: their conditions are prefilters, not violations.
