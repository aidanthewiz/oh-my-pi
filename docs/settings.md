...
| Environment overlay | Any file listed in `PI_CONFIG_FILES` | Loaded after global and project settings, in platform path-list order, for this process only. | Never persisted. |
| CLI overlay | Any file passed with `--config <file>` | Loaded after environment overlays, for that one process. Repeatable. | Never persisted. |
| Managed overlay | `<agentDir>/config.managed.yml` | Loaded when present, above global, project, and config overlays. Org-managed distributions ship policy alongside the profile. | Never written by the engine; owned by distribution tooling. |
...
The catalog below highlights common settings; it is not the complete schema. `coreforge config list` is the authoritative reference for every key, current value, type, and description. Defaults and enum values shown here come from the schema. Settings that accept an env or flag override are noted; those overrides are process-local and not persisted.
...
Every schema path not individually tabulated in this catalog is explicitly deferred to `coreforge config list`. Additional groups include:
