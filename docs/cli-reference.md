...
`coreforge` is invoked as:

```sh
coreforge [command] [flags] [messages...]
```

When the first non-flag argument is **not** a registered subcommand, `coreforge`
routes to the default [`launch`](#launch-the-default-command) command and treats
the arguments as the initial prompt. So `coreforge "fix the build"` launches a
session with that message, while `coreforge models` runs the `models` subcommand.

Runtime help is also available:

- `coreforge --help` lists user-facing subcommands and common launch flags.
- `coreforge <command> --help` prints that command's public flags and examples.

This page is the consolidated reference for the shared **launch surface** (the
flags accepted by `coreforge` / `coreforge launch`) and every top-level **subcommand**.
Per-subcommand flags (for example `coreforge auth-broker --json`) are documented
by each command's `--help`.

## Launch (the default command)

`coreforge` and `coreforge launch` start a coding session. Positional arguments become the
initial message(s):
...
coreforge "List all .ts files in src/"

# Attach files/images to the initial message (prefix with @)
coreforge @prompt.md @image.png "What color is the sky?"

# Non-interactive: process the prompt and exit (headless / print mode)
coreforge -p "List all .ts files in src/"

# Continue the previous session
coreforge --continue "What did we discuss?"
```
...
| `--from-claude` | Import a Claude Code session into Coreforge. |
| `--from-codex` | Import a Codex session into Coreforge. |
...
| `--no-mcp` | Disable MCP discovery and connections entirely for this run. |
| `--mcp-project-config` | Load project-level MCP config (`.mcp.json`) for this run, overriding managed policy. |
| `--mcp-providers <a,b,c>` | Use a comma-separated MCP discovery provider allowlist for this run. |
...
| Flag | Description |
| --- | --- |
| `--help`, `-h` | Show help for `coreforge` or a subcommand and exit. |
| `--version`, `-v` | Print the installed version and exit. |

### Headless / print mode

`--print` / `-p` runs `coreforge` non-interactively: it processes the prompt, streams
the result to stdout, and exits without entering the TUI. This is the entry point
for scripting and automation.

```sh
# Print the answer and exit
coreforge -p "Summarize the changes in the last commit"

# Include the model's thinking blocks in the printed text
coreforge -p --print-thoughts "Explain your reasoning for this refactor"

# Machine-readable output for pipelines
coreforge -p --mode json "List every TODO in src/" > todos.json

# Pipe a prompt via stdin
echo "review this diff" | coreforge -p
```
...
Run `coreforge <command> --help` for each command's own flags and examples.

| Command | Purpose | See also |
| --- | --- | --- |
| `launch` | Start a coding session (the default command). | [Launch flags](#launch-flags) |
| `acp` | Run Coreforge as an ACP (Agent Client Protocol) server over stdio. | [approval mode](./approval-mode.md#acp-sessions) |
| `auth-broker` | Manage the Coreforge auth broker (credential vault). | [auth broker / gateway](./auth-broker-gateway.md) |
...
| `mcp` | Configure MCP server selections. | |
...
| `relay` | Approve a browser for Coreforce Agent Collab. | |
