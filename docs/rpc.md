...
The `hub start` tool therefore defaults `--mode rpc` and `--mode rpc-ui` processes to pipes. An explicit `pty: true` overrides the default and remains subject to the platform canonical-line safety limit.
