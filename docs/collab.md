...
`coreforge collab list` (and `/collab list` inside a TUI) enumerates every live Collab host on the local machine under the same Coreforge configuration root — across terminals, projects, and profiles. Listing is metadata only; it never prints or transmits a link:

```
coreforge collab list                          # one row per host, no links
coreforge collab list --json                   # {"version": 1, "hosts": [...]}
coreforge collab link <instanceId|pid>         # print that host's full-control browser URL
coreforge collab link <instanceId|pid> --view  # print its view-only browser URL
coreforge collab link <instanceId> --json      # {"version": 1, "instanceId", "generation", "access", "url"}
```
...
Accepted by `/join <link>` and `coreforge join "<link>"`:
