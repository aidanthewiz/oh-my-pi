...
describe("omp-stats --json", () => {
	it("writes parseable JSON without a human sync summary", async () => {
		const proc = Bun.spawn([process.execPath, cliEntry, "--json"], {
			stdout: "pipe",
			stderr: "pipe",
			env: {
				...process.env,
				...
				HOME: tempDir.path(),
				XDG_CACHE_HOME: path.join(tempDir.path(), "xdg-cache"),
				XDG_DATA_HOME: path.join(tempDir.path(), "xdg-data"),
				XDG_STATE_HOME: path.join(tempDir.path(), "xdg-state"),
				NO_COLOR: "1",
