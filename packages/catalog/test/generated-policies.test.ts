...
describe("generated model policies", () => {
	...
	it("pins first-party GPT-5.6 prices without overwriting Bedrock Mantle prices", () => {
		...
		expect(built[1]?.cost).toMatchObject({ input: 2, output: 12, cacheRead: 0.2, cacheWrite: 2.5 });
		expect(built[2]?.cost).toEqual(mantleNative);
		expect(built[3]?.cost).toEqual(stale);
	});
