 ...
 * `coreforge`. Embedders that drive Coreforge programmatically set the env var
 * so broker-side per-client burn tracking can answer "what did app X use"
 * instead of folding everything into one install-wide bucket.
 */
export function getAppName(): string {
	const value = process.env.OMP_APP_NAME?.trim();
	return value ? value : "coreforge";
}
