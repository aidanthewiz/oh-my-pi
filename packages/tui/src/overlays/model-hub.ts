...
export class ModelHubComponent implements Component {
	...
	#reloadRoles(autoCandidates: ReadonlyArray<Model>): void {
		const allModels =
			this.#scopedModels.length > 0
				? autoCandidates
				: (this.#settings.scopeModels?.(this.#registry.getAll("all")) ?? this.#registry.getAll("all"));
	...
	#syncFromRegistryState(): void {
		...
		if (this.#scopedModels.length > 0) {
			...
			allModels = this.#settings.scopeModels?.(this.#registry.getAll("all")) ?? this.#registry.getAll("all");
			try {
				availableModels =
					this.#settings.scopeModels?.(this.#registry.getAvailable("all")) ?? this.#registry.getAvailable("all");
			} catch (error) {
				this.#configError = error instanceof Error ? error.message : String(error);
				availableModels = [];
			}
