...
export class ModelHubComponent implements Component {
	...
	#reloadRoles(autoCandidates: ReadonlyArray<Model>): void {
		const allModels =
			this.#scopedModels.length > 0
				? autoCandidates
				: (this.#settings.scopeModels?.(this.#registry.getAll()) ?? this.#registry.getAll());
	...
	#syncFromRegistryState(): void {
		...
		if (this.#scopedModels.length > 0) {
			...
			allModels = this.#settings.scopeModels?.(this.#registry.getAll()) ?? this.#registry.getAll();
			try {
				availableModels =
					this.#settings.scopeModels?.(this.#registry.getAvailable()) ?? this.#registry.getAvailable();
			} catch (error) {
				this.#configError = error instanceof Error ? error.message : String(error);
				availableModels = [];
			}
