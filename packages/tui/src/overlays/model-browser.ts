...
function scopeModels(source: ModelBrowserSource, models: ReadonlyArray<Model>): Model[] {
	return source.scopeModels ? source.scopeModels(models) : [...models];
}
...
export function resolveRoleAssignments(
	...
	const catalog = scopeModels(settings, allModels);
...
export function buildSessionModelScope(
	...
	if (scopedModels.length > 0) {
		...
		try {
			models = scopeModels(settings, registry.getAvailable());
		} catch (cause) {
