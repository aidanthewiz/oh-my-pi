...
export class OAuthSelectorComponent extends OverlayPanel {
	...
	constructor(
		...
		options?: {
			...
			/** Host-scoped provider list. Defaults to the complete OAuth registry. */
		...
		this.#loadProviders(options?.disabledProviders, options?.providers);
	...
	#loadProviders(
		disabledProviders: readonly string[] = [],
		providers: readonly OAuthProviderInfo[] = getOAuthProviders(),
	): void {
		if (this.#mode === "logout") {
