...
export class OpenRouterOAuthFlow extends OAuthCallbackFlow {
	...
	async generateAuthUrl(_state: string, redirectUri: string): Promise<{ url: string; instructions?: string }> {
		...
		return {
			...
			instructions:
				"Authorize in your browser, or paste an existing OpenRouter API key (sk-or-…) when prompted. If the browser cannot reach this machine, paste the final redirect URL or authorization code instead.",
		};
	}
