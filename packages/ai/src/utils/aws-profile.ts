...
export function resolveAwsProfile(profile?: string): string {
	const explicitProfile = allowAmbientAwsModelCredentials() ? profile : undefined;
	return explicitProfile || resolveAwsModelProfile() || "default";
}
...
export function shouldLoadAwsSharedConfig(profile?: string): boolean {
	if ((allowAmbientAwsModelCredentials() && profile) || resolveAwsModelProfile()) return true;
...
export function resolveAwsProfileRegion(profile?: string): string | undefined {
	...
	const configPath =
		(allowAmbientAwsModelCredentials() && $env.AWS_CONFIG_FILE) || path.join(os.homedir(), ".aws", "config");
...
export function hasConfiguredAwsProfile(profile?: string): boolean {
	...
	const credentialsPath =
		(allowAmbientCredentials && $env.AWS_SHARED_CREDENTIALS_FILE) || path.join(os.homedir(), ".aws", "credentials");
	const configPath = (allowAmbientCredentials && $env.AWS_CONFIG_FILE) || path.join(os.homedir(), ".aws", "config");
