...
function resolveAnthropicControlBetas(
	...
	if (supportsMidConversationSystem) betas.push(midConversationSystemBeta);
		...
		try {
			...
			let prefixMismatchBehavior =
				model.thinking?.prefixBinding && model.compat.supportsThinkingBindingControls
					...
					: undefined;
			const controlBetas = resolveAnthropicControlBetas(model, prefixMismatchBehavior, supportsMidConversationSystem);
			...
			if (options?.client) {
				...
				if (
					...
					getCacheControl(model, options?.cacheRetention, baseUrl).cacheControl?.ttl === "1h" &&
...
function buildParams(
	...
	const effectiveModel =
		forceDemoteUnsignedThinking && model.compat.replayUnsignedThinking
			...
			: model;
	const { cacheControl } = getCacheControl(model, options?.cacheRetention, effectiveBaseUrl);
