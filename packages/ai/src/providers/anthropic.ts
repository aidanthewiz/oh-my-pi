...
function getCacheControl(
	...
	// OAuth follows Claude Code's one-hour default only on endpoints that
	// advertise that capability. API keys and incompatible routes stay short.
	const defaultRetention = isOAuthToken && supportsLongCacheRetention ? "long" : "short";
...
function resolveAnthropicBaseUrl(model: Model<"anthropic-messages">, apiKey?: string): string | undefined {
	...
	if (!resolved && model.provider === "anthropic") {
		resolved = resolveDirectAnthropicBaseUrl(model);
...
const streamAnthropicOnce = (
	...
	(async () => {
		...
		try {
			...
			// Keep fallback payloads aligned with the top-level effort gate:
			// no nested effort field means the fallback scan cannot re-add its beta.
			let fallbacks = isOfficialAnthropicApiUrl(baseUrl) ? options?.fallbacks : undefined;
			if (
			...
			if (options?.client) {
				...
				// requests. OAuth requests never add it here: Anthropic accepts the
				// TTL without changing the Claude Code header fingerprint.
				if (
					!effectiveCredentialIsOAuth &&
					getCacheControl(model, options?.cacheRetention, baseUrl, effectiveCredentialIsOAuth).cacheControl
						?.ttl === "1h" &&
...
type AnthropicParamBuildOptions = {
	...
	/** Whether the effective endpoint accepts server-side compaction. */
	compactionSupported: boolean;
};
const ANTHROPIC_AWS_INFERENCE_GEO_MIN_REVISION = parseRevision("4.6");
...
function applyAnthropicAwsInferenceGeo(
	...
	if (
		...
		revision === undefined ||
		ANTHROPIC_AWS_INFERENCE_GEO_MIN_REVISION === undefined ||
		compareRevision(revision, ANTHROPIC_AWS_INFERENCE_GEO_MIN_REVISION) < 0
	) {
		return;
	}
...
function buildParams(
	...
	const effectiveModel =
		forceDemoteUnsignedThinking && model.compat.replayUnsignedThinking
			...
			: model;
	const { cacheControl } = getCacheControl(model, options?.cacheRetention, effectiveBaseUrl, isOAuthToken);
	...
	systemBlocks = planStableAnthropicSystem(
		systemBlocks,
		controlState,
		supportsMidConversationSystemAtEndpoint(model, effectiveBaseUrl),
	);
...
export function convertAnthropicMessages(
	...
	const supportsMidConversationSystem =
		opts?.effectiveBaseUrl === undefined
			...
			: supportsMidConversationSystemAtEndpoint(model, opts.effectiveBaseUrl);
	if (developerParams.length > 0 && supportsMidConversationSystem) {
