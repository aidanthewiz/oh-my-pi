 ...
 * `us-gov.`), the selected model-auth or ambient AWS region is honored only
 * when it can serve the profile's geo. If it is absent or mismatched, a
 * same-geo guardrail ARN region is used when available; otherwise the geo
 * default is used. `global.` profiles have no geo entry, so the selected or
 * ambient region (or, when absent, a guardrail ARN's region or
