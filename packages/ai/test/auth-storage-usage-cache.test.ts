...
describe("AuthStorage usage cache: explicit invalidation", () => {
	...
	it("refreshes a cached Codex usage report after redeeming a saved reset", async () => {
		const store = makeStore([
			{
				id: 1,
					...
					limits: [
						{
							...
							amount: { used: usageCalls === 1 ? 100 : 0, limit: 100, unit: "percent" },
							status: "ok",
						},
					],
					metadata: { accountId: "account-reset", email: "reset@example.com" },
				};
			},
		};
		const storage = new AuthStorage(store, {
		...
		try {
			...
			expect((await storage.fetchUsageReports())?.[0]?.limits[0]?.amount.used).toBe(0);
