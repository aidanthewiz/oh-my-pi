...
beforeEach(() => {
	previousAnthropicBaseUrl = Bun.env.ANTHROPIC_BASE_URL;
	delete Bun.env.ANTHROPIC_BASE_URL;
});

afterEach(() => {
	vi.restoreAllMocks();
	if (previousAnthropicBaseUrl === undefined) delete Bun.env.ANTHROPIC_BASE_URL;
	else Bun.env.ANTHROPIC_BASE_URL = previousAnthropicBaseUrl;
});
