...
describe("daemon broker lease recovery", () => {
	it(
		...
		async () => {
			...
			const pidPath = path.join(daemonBrokerRuntimeDir(runtimeRoot), "broker.pid");
			try {
				await fs.mkdir(daemonBrokerRuntimeDir(runtimeRoot), { recursive: true });
				...
				await withScope(path.join(root, "project"), runtimeRoot, async client => {
