...
describe("CLI command help metadata", () => {
	...
	it("advertises the Coreforge MCP escape hatches in root and launch help", async () => {
		const metadata = new Map<string, CommandMetadata>();
		for (const entry of commands) {
			if (entry.help) metadata.set(entry.name, entry.help);
		}

		const rootOutput = captureStdout(() => renderRootHelp({ bin: "coreforge", version: "test", commands: metadata }));
		const launch = commands.find(entry => entry.name === "launch");
		if (!launch) throw new Error("Missing launch command");
		const LaunchCommand = await launch.load();
		const launchOutput = captureStdout(() => renderCommandHelp("coreforge", "launch", LaunchCommand));

		for (const flag of ["--no-mcp", "--mcp-project-config", "--mcp-providers=<value>"]) {
			expect(rootOutput).toContain(flag);
			expect(launchOutput).toContain(flag);
		}
	});
});
