...
describe("daemon broker", () => {
	...
	it("keeps a valid RPC response authoritative after a malformed completion", async () => {
			...
			socket.on("data", chunk => {
				...
				socket.write(
					`${JSON.stringify({
						id: request.id,
						ok: true,
						result: { projectDir, protocolVersion: DAEMON_BROKER_PROTOCOL_VERSION },
					})}\n`,
				);
			});
		});
		...
		server.listen(daemonBrokerEndpoint(projectDir, daemonBrokerRuntimeDir(runtimeDir)), listening.resolve);
		await listening.promise;
		try {
			expect(await client.request({ op: "ping" })).toEqual({
				op: "ping",
				projectDir,
				protocolVersion: DAEMON_BROKER_PROTOCOL_VERSION,
			});
	...
	it("replays a zero-width completion without poisoning the next start", async () => {
		...
		try {
			...
			await first
				...
				.catch(() => undefined);
			const metaPath = path.join(daemonBrokerRuntimeDir(runtimeDir), "daemons", "empty-ready-poison", "meta.json");
			expect(
				await waitUntil(async () => {
