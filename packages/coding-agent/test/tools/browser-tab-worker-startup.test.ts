...
describe("browser tab worker startup", () => {
	...
	it("resolves with ready info when the worker sends setup before ready", async () => {
		...
		worker.emitSetup();
		// `setup` and `ready` can land in the same tick; the single listener
		// spanning both phases must resolve it instead of dropping it.
	...
	it("bounds initialization by the caller's remaining budget", async () => {
		const worker = new FakeStartupWorker();
		// Simulate 25 s already consumed from the caller's 30 s budget.
	...
	it.skipIf(!CHROMIUM_AVAILABLE)(
		...
		async () => {
			...
			try {
				...
				// `timeoutMs + GRACE_MS` clock. The exhausted budget must surface
				// the original isolated-worker error.
				...
				expect(String((failure as Error).message)).toContain("Failed to start isolated browser tab worker");
				// Only one attempt's floors are spent.
