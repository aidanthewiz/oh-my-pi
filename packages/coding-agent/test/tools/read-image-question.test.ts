...
describe("read image questions", () => {
	...
	it("does not select a vision model outside the enabledModels allowlist", async () => {
		const settings = Settings.isolated({ enabledModels: ["openai/gpt-5-vision"] });
		settings.setModelRole("vision", "openai/gpt-4o");
		const stub = createCompleteSimpleSuccessStub("Allowed model");
		const session = createSession(testDir, visionModel, "test-key", settings, {
			configureVisionRole: false,
			availableModels: [visionModel, reasoningVisionModel],
			activeModel: visionModel,
		});

		await new ReadTool(session, stub.fn).execute("call", { path: `${imagePath}?q=What is visible?` });

		const selectedModel = stub.calls[0]?.[0] as Model<"openai-responses"> | undefined;
		expect(selectedModel?.id).toBe(reasoningVisionModel.id);
	});
