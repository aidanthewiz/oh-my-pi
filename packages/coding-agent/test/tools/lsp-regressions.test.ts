...
interface FakeLspServer {
	...
	/** Environment passed to the most recent language-server spawn. */
	readonly spawnedEnv: Readonly<Record<string, string | undefined>> | undefined;
...
function installFakeLsp(handler: FakeLspHandler, options?: FakeLspOptions): FakeLspServer {
	...
	let killed = false;
	let spawnCount = 0;
	let spawnedEnv: Readonly<Record<string, string | undefined>> | undefined;
	...
	vi.spyOn(piUtils.ptree, "spawn").mockImplementation(((_command, options) => {
		spawnCount++;
...
describe("lsp regressions", () => {
	...
	it("filters managed profile secrets from language server processes", async () => {
		const tempDir = TempDir.createSync("@omp-lsp-client-env-");
		...
		try {
			...
			expect(server.spawnedEnv?.OPENAI_API_KEY).toBeUndefined();
			expect(server.spawnedEnv?.PATH).toBe(Bun.env.PATH);
		} finally {
			await lspClient.shutdownAll();
			tempDir.removeSync();
