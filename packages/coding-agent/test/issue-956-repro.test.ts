...
describe("interactive /mcp test", () => {
	...
	it("tests a discovered server and keeps its advertised Esc cancellation grace", async () => {
		...
		const prepareConfig = vi.fn(async config => config);
		const mcpTestEscapeHandlers = new Set<() => void>();
		const { controller, ctx } = createController({ mcpTestEscapeHandlers, mcpManager: { prepareConfig } });
		...
		expect(prepareConfig).toHaveBeenCalledWith(expect.objectContaining({ command: "github-mcp-server" }), {
			sourceLevel: "project",
		});
