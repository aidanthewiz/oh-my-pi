...
describe("ChainJudge", () => {
	...
	it("does not append a session model excluded by the effective model scope", async () => {
		const settings = Settings.isolated({
			modelRoles: { judge: `${ONLINE_BACKUP.provider}/${ONLINE_BACKUP.id}` },
			disabledModels: [`${ONLINE.provider}/${ONLINE.id}`],
		});
		const registry = makeRegistry([ONLINE_BACKUP, ONLINE], { [ONLINE.provider]: "online-key" });
		const attempted: string[] = [];
		vi.spyOn(ai, "completeSimple").mockImplementation(async model => {
			attempted.push(model.id);
			return reply(model, "level: low");
		...
		expect(result.answers.level.choice).toBe("low");
		expect(attempted).toEqual([ONLINE_BACKUP.id]);
	});
