...
describe("OpenCode MCP discovery", () => {
	...
	test("expands OpenCode {env:VAR} and {file:path} substitutions", async () => {
		const userConfigDir = path.join(tempDir, ".config", "opencode");
		await fs.mkdir(userConfigDir, { recursive: true });
		const secretFile = path.join(userConfigDir, "mcp-key.txt");
	...
	test("keeps project substitutions inside the project trust boundary", async () => {
		const projectDir = path.join(tempDir, "project");
		const secretName = `OMP_TEST_OPENCODE_SECRET_${crypto.randomUUID().replaceAll("-", "")}`;
		await fs.mkdir(projectDir);
		await fs.writeFile(path.join(projectDir, "inside-token.txt"), "inside-token\n");
		await fs.writeFile(path.join(tempDir, "outside-token.txt"), "outside-token\n");
		await fs.symlink(path.join(tempDir, "outside-token.txt"), path.join(projectDir, "linked-token.txt"));
		await fs.writeFile(
			path.join(projectDir, "opencode.json"),
			JSON.stringify({
				mcp: {
					bounded: {
						...
						url: "https://mcp.example.xyz/mcp",
						headers: {
							"X-Allowed-Env": "{env:HOME}",
							"X-Secret-Env": `{env:${secretName}}`,
							"X-Inside-File": "{file:./inside-token.txt}",
							"X-Outside-File": "{file:../outside-token.txt}",
							"X-Symlink-File": "{file:./linked-token.txt}",
						},
					},
				},
			}),
		...
		try {
			const servers = await loadOpenCodeMcpConfig(projectDir);
			expect(servers.find(server => server.name === "bounded")?.headers).toEqual({
				"X-Allowed-Env": Bun.env.HOME ?? "",
				"X-Secret-Env": "",
				"X-Inside-File": "inside-token",
				"X-Outside-File": "",
				"X-Symlink-File": "",
			});
		} finally {
			delete Bun.env[secretName];
		}
	});
});
