...
import { resolveRoleChain, roleCandidatePool } from "@oh-my-pi/pi-coding-agent/config/model-resolver";
...
describe("ModelRegistry", () => {
	...
	describe("model kind pools", () => {
		...
		test("applies the effective model scope to kind-role candidate pools", () => {
			const scopedSettings = Settings.isolated({ enabledModels: ["web/duckduckgo"] });
			const registry = new ModelRegistry(authStorage, modelsJsonPath, { settings: scopedSettings });

			expect(
				roleCandidatePool("web", scopedSettings, registry).map(model => `${model.provider}/${model.id}`),
			).toEqual(["web/duckduckgo"]);
			expect(roleCandidatePool("judge", scopedSettings, registry)).toEqual([]);
		});
	});
	...
	describe("route-aware Anthropic availability", () => {
		...
		test("preserves configured credentials for an explicit Anthropic AWS gateway override", async () => {
			...
			try {
				...
				expect((await registry.resolveModelHeaders(model))?.["Anthropic-Workspace-Id"]).toBe("wrkspc_model");
			} finally {
				for (const name of names) {
					const value = previous[name];
					if (value === undefined) delete Bun.env[name];
