...
import { $env, filterChildShellEnv, getProjectDir, isEnoent, prompt } from "@oh-my-pi/pi-utils";
...
async function runSingleCommit(proposal: CommitProposal, ctx: CommitExecutionContext): Promise<void> {
	...
	try {
		await repo.commitCreate(commitMessage, { env: filterChildShellEnv(Bun.env, ctx.cwd) });
	} catch (error) {
...
async function runSplitCommit(
	...
	await repo.unstage([]);
	const commitEnv = filterChildShellEnv(Bun.env, ctx.cwd);
	for (const [position, commitIndex] of order.entries()) {
		...
		const message = formatCommitMessage(analysis, commit.summary);
		try {
			await repo.commitCreate(message, { env: commitEnv });
		} catch (error) {
