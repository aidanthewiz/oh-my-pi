...
import { filterChildShellEnv, getProjectDir } from "@oh-my-pi/pi-utils";
...
async function runLegacyCommitCommand(args: CommitCommandArgs): Promise<void> {
	...
	try {
		await vcs.requireGit(cwd).commitCreate(commitMessage, { env: filterChildShellEnv(Bun.env, cwd) });
	} catch (error) {
