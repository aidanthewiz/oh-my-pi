...
/** Environment visible to user- or project-owned imported configuration. */
export function environmentForConfigLevel(level: "user" | "project"): Record<string, string | undefined> {
	return level === "project" ? projectConfigEnvironment() : Bun.env;
}
...
export async function listClaudePluginRoots(
	...
	const cacheKey = `${claudeConfigDir}:${ompRegistryPath}:${resolvedProjectPath ?? ""}:${activeClaudeProjectPath ?? ""}:${canonicalCwd ?? ""}:${enabledOverrides.sources.join("|")}:${persistentPluginPolicyCacheKey()}`;
