...
export async function discoverSessionExtensionPaths(
	...
	return withPersistentPluginPolicy(policy, async () => {
		...
		const configured = roots?.configured ?? settings.get("extensions") ?? [];
		const configuredPaths = explicitOnly
			? [...explicit]
			: [...explicit, ...filterPersistentExtensionPaths(configured, cwd)];
...
async function createAgentSessionScoped(options: CreateAgentSessionOptions): Promise<CreateAgentSessionResult> {
	...
	// Build the obfuscator before loading resumed state. Credential-marked
	// settings remain protected even when configurable secret scanning is off.
	const obfuscator: SecretObfuscator | undefined = await buildSecretObfuscator(cwd, agentDir, {
		keyDir: options.agentDir,
		settings,
		keepAvailable: true,
	});
	...
	let stopImageUrlService: (() => void) | undefined;
	...
	try {
		...
		let refreshedAllowedModels = await resolveAllowedModels(modelRegistry, settings, modelMatchPreferences);
		if (!hasExplicitModel && sessionRetryLimit > 0) {
			...
			if (!restoreSessionModel()) {
				...
				if (candidateProviders.size > 0) {
					...
					refreshedAllowedModels = await resolveAllowedModels(modelRegistry, settings, modelMatchPreferences);
					restoreSessionModel();
				}
			}
		}
		...
		if (!model && deferredModelPatterns.length > 0) {
			...
			const scopedAllModels = filterModelsByConfiguredScope(modelRegistry.getAll(), settings);
			const allEnabledModels =
				disabledProviders.size === 0
					? scopedAllModels
					: scopedAllModels.filter(candidate => !disabledProviders.has(candidate.provider));
			const scopedAvailableModels = await resolveAllowedModels(modelRegistry, settings, matchPreferences);
			const availableModels =
				disabledProviders.size === 0
					? scopedAvailableModels
					: scopedAvailableModels.filter(candidate => !disabledProviders.has(candidate.provider));
		...
		blobBroker?.prewarm();
		if (blobBroker) {
			stopImageUrlService = () => blobBroker.stop();
			disposeCallbacks.add(stopImageUrlService);
		}
