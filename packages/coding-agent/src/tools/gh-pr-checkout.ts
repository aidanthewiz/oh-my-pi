...
import { filterGitChildShellEnv, getWorktreeDir, hashPath, isEnoent, logger } from "@oh-my-pi/pi-utils";
	...
	return withRepoLock(
		...
		async () => {
			...
			await repository.fetch(
				...
				PR_FETCH_TIMEOUT_MS,
				signal,
				filterGitChildShellEnv(Bun.env, repoRoot),
			);
	...
	await repository.push(
		{
			...
			env: filterGitChildShellEnv(Bun.env, repoRoot),
		},
		signal,
	);
