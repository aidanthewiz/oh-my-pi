...
async function acquireTabImpl(
	...
	try {
		...
		// A killed worker cannot close a page created before startup failed.
...
async function initializeTabWorker(
	...
	// Derive both phase budgets from the remaining caller budget so time spent
	// before initialization cannot restart the caller's timeout.
