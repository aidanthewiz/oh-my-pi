...
import { filterChildShellEnv, formatBytes, getWorktreeDir, logger, Snowflake } from "@oh-my-pi/pi-utils";
	...
	for (const [relativePath, repoPatches] of byRepo) {
		...
		try {
			...
			if (await repository.isDirty()) {
				...
				await repository.commitCreate(msg, { env: filterChildShellEnv(Bun.env, nestedDir) });
			}
		} finally {
			if (stashed) {
...
async function commitPatchToBranchWorktree(
	...
	await repo.commitCreate(message, {
		...(author ? { author } : {}),
		env: filterChildShellEnv(Bun.env, tmpDir),
	});
}
	...
	if (agentCommitted) {
		if (baselineHasRootWip(baseline.root)) {
			...
			await repo.fetch(
				...
				undefined,
				filterChildShellEnv(Bun.env, repoRoot),
			);
