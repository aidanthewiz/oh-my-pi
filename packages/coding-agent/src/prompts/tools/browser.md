...
- `app.relay: true` → drive the user's own Chrome tabs via the Coreforge browser relay (auto-started; needs the Coreforge Browser Relay extension installed). `app.target` picks a tab by URL/title substring; without it the visible tab is adopted without stealing focus.
