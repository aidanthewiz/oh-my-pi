...
{{#if spawns}}agent(prompt, agent?="{{spawnDefaultAgent}}", label?=None, schema?=None, schema{{#if js}}Mode{{else}}_mode{{/if}}?="permissive", context{{#if js}}FilePolicy{{else}}_file_policy{{/if}}?="inherit", isolated?=None, apply?=None, merge?=None, handle?=False) → str | dict
...
{{#if js}}    JS: ONE trailing object — agent(prompt, { agent, label, schema, schemaMode, contextFilePolicy, isolated, apply, merge, handle }).{{/if}}
{{/if}}
