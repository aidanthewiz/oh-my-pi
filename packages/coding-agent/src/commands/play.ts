...
export default class Play extends Command {
	...
	static examples = [
		`${CF_COMMAND} play`,
		`${CF_COMMAND} play /tmp/omp-recordings/2026-09-22T10-00-00-1a2b3c4d.ompcast -s 2 -i 1`,
	];
	...
	async run(): Promise<void> {
		...
		if (!process.stdout.isTTY) throw new CliUsageError(`${CF_COMMAND} play needs an interactive terminal`);
