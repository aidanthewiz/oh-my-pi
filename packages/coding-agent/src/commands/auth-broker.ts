...
export default class AuthBroker extends Command {
	...
	static examples = [
		`# Boot the broker against the local SQLite store\n  ${CF_COMMAND} auth-broker serve`,
		`# Boot on a non-default port\n  ${CF_COMMAND} auth-broker serve --bind=127.0.0.1:9000`,
		`# Print the bearer token\n  ${CF_COMMAND} auth-broker token`,
		`# Rotate the bearer token\n  ${CF_COMMAND} auth-broker token --regenerate`,
		`# List supported OAuth providers\n  ${CF_COMMAND} auth-broker list`,
		`# Local login (run on the broker host)\n  ${CF_COMMAND} auth-broker login anthropic`,
		`# Interactive provider selection\n  ${CF_COMMAND} auth-broker login`,
		`# Remote login over SSH tunnel\n  ${CF_COMMAND} auth-broker login anthropic --via=user@broker`,
		`# Log out of a provider (interactive without provider arg)\n  ${CF_COMMAND} auth-broker logout anthropic`,
		`# Import a CLIProxyAPI auth dump\n  ${CF_COMMAND} auth-broker import ~/.cliproxy/auth`,
		`# Import a single CLIProxyAPI JSON, overriding the provider mapping\n  ${CF_COMMAND} auth-broker import ~/.cliproxy/auth/claude-foo.json --provider anthropic`,
		`# Preview a migration from local store + env vars to the configured broker\n  ${CF_COMMAND} auth-broker migrate --from-local --include-env --dry-run`,
		`# Apply the migration\n  ${CF_COMMAND} auth-broker migrate --from-local --include-env`,
		`# Health-check the configured remote broker\n  ${CF_COMMAND} auth-broker status`,
	];

	async run(): Promise<void> {
		const { args, flags } = await this.parse(AuthBroker);
		if (!args.action) {
			renderCommandHelp(CF_COMMAND, "auth-broker", AuthBroker);
			return;
		}
