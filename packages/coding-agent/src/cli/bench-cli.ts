...
export async function runBenchCommand(command: BenchCommandArgs, deps: BenchDependencies = {}): Promise<BenchSummary> {
	...
	if (command.models.length === 0) {
		throw new Error(`Pass at least one model selector, e.g. \`${CF_COMMAND} bench opus gpt-5.2\``);
	}
		...
		for (const { selector, model, thinking } of targets) {
			...
			if (!preflightKey) {
				const failure: BenchRunFailure = {
					ok: false,
					error: `No credentials for provider "${model.provider}". Run \`${CF_COMMAND}\` and use /login, or set the provider API key.`,
				};
