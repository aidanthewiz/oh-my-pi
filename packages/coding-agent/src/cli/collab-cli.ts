...
/** Versioned top-level JSON shape for `coreforge collab list --json`. */
export interface CollabListJsonOutput {
	version: number;
	hosts: CollabHostSnapshot[];
}

/** Versioned capability response for `coreforge collab link --json`. */
...
export async function runCollabListCommand(
	...
	print(chalk.dim(`Get a link: ${CF_COMMAND} collab link <instanceId|pid> [--view]`));
}
