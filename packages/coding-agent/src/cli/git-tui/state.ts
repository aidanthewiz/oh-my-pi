...
import { BINARY_SNIFF_BYTES, filterChildShellEnv, isEnoent, isProbablyBinaryHeader } from "@oh-my-pi/pi-utils";
...
export class GitModel {
	...
	async commit(message: string, options: { amend?: boolean } = {}): Promise<void> {
		await this.#repo.commitCreate(message, {
			amend: options.amend,
			env: filterChildShellEnv(Bun.env, this.cwd),
		});
	}
