...
export const BUILTIN_COLLABORATION_SLASH_COMMANDS: ReadonlyArray<SlashCommandSpec> = [
	...
	},
	{
		...
		handle: async (_command, runtime) => {
			try {
				...
				lines.push(`Browser approval: ${CF_COMMAND} relay authorize <code>`);
				await runtime.output(lines.join("\n"));
				return commandConsumed();
			} catch (err) {
	...
	},
	{
		...
		subcommands: [
			...
			{ name: "list", description: `List active local Collab hosts (no links; use \`${CF_COMMAND} collab link\`)` },
		...
		handleTui: async (command, runtime) => {
			...
			if (verb === "list") {
				// Same registry as `coreforge collab list`: metadata only, never a link. A
				// link is a deliberate per-host act (`coreforge collab link <id> [--view]`),
				// so a listing can be shown or logged without granting anything.
				if (rest.trim()) {
					ctx.showError(`Usage: /collab list — for links or JSON use \`${CF_COMMAND} collab link|list\``);
					return;
				}
				...
				for (const host of hosts) {
					...
					lines.push(
						...
						`   ${theme.fg("dim", `${CF_COMMAND} collab link ${host.instanceId}${host.access === "view" ? " --view" : ""}`)}`,
					);
				}
