/**
 * Session recordings (`/record` and CLI playback).
 *
 * A recording is the single-pane Coreforge stream feed persisted to disk: the
 * same normalized, redacted screen frames {@link StreamPaintEncoder} produces for
...
export interface RecordingHeader {
	...
	title: string;
	createdAt: string;
	/** Clip description, set by the clip command's `--description` option. */
	description?: string;
...
export function parseRecording(text: string): Recording {
	...
	if (!isRecordingHeader(header)) throw new Error("not an ompcast recording (missing ompcast header)");
