...
const AUTH_CHALLENGE_PARAM_PREFIX = /^[!#$%&'*+.^_`|~0-9A-Za-z-]+=(?:"|[^=\s,;])/;
...
function redactAuthCredential(match: string, scheme: string, value: string): string {
	return AUTH_CHALLENGE_PARAM_PREFIX.test(value) ? match : `${scheme} [redacted]`;
}

function sanitizeDiagnosticText(value: string, maxChars: number): string {
	return value
		.replace(FETCH_VERBOSE_ADVICE, "")
		.replace(AUTH_SCHEME_VALUE, redactAuthCredential)
