...
import { CF_COMMAND, CF_DISPLAY_VERSION, CF_VERSION } from "./cli/cf-version";
...
export async function runCli(argv: string[]): Promise<void> {
	...
	if (
		...
		beginStartupComposer({ version: CF_DISPLAY_VERSION });
	...
	try {
		...
		await run({ bin: CF_COMMAND, version: CF_VERSION, argv: resolved.argv, commands, metadataHelp: showHelp });
	} finally {
		stopStartupComposer?.();
	}
}
