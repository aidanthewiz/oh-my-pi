...
export class InteractiveMode implements InteractiveModeContext {
	...
	constructor(
		...
		this.composer =
			...
			new Composer({
				...
				welcome: {
					version,
					productName: CF_BRAND,
					greetingName: getCoreforgeGreetingName(),
	...
	async init(options: InteractiveModeInitOptions = {}): Promise<void> {
		...
		// Get current model info for the startup composer.
	...
	stop(): void {
		...
		// Clear the process-global report handler so it cannot outlive this session.
	...
	async toggleRecording(): Promise<void> {
		...
		if (active) {
			...
			this.showStatus(
				`Saved ${formatDuration(elapsed)} recording to ${active.path} · replay: ${CF_COMMAND} play · share: ${CF_COMMAND} clip`,
			);
			return;
		}
