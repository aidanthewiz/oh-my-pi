...
export class UiHelpers {
	...
	showNewVersionNotification(newVersion: string): void {
		...
		block.addChild(new Text("Update Available", 1, 0).setStyleFn(text => theme.bold(theme.fg("warning", text))));
		block.addChild(
			new Text(`New version ${newVersion} is available.`, 1, 0).setStyleFn(text => theme.fg("muted", text)),
		);
		block.addChild(
			new Text("This is a terminal command, not a chat command. Exit Coreforge, then run:", 1, 0).setStyleFn(text =>
				theme.fg("muted", text),
			),
		);
		// The Coreforge wrapper also re-syncs org config; `omp update` does not.
		block.addChild(new Text("coreforge update", 1, 0).setStyleFn(text => theme.fg("accent", text)));
		block.addChild(new DynamicBorder(text => theme.fg("warning", text)));
