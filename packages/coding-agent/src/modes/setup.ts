...
function webRoleModels(ctx: InteractiveModeContext) {
	return filterModelsByConfiguredScope(ctx.session.modelRegistry.getAll("all"), ctx.settings).filter(
		getRoleInfo("web", ctx.settings).accepts,
	);
}
...
export function createSetupHost(ctx: InteractiveModeContext): SetupHost {
	...
	return {
		...
		get oauthProviders() {
			return filterOAuthLoginProviders(getOAuthProviders());
		},
		get authStorage() {
		...
		modelSource,
		getModels: () => ({
			available: getAllowedAvailableModels(ctx.session.modelRegistry, ctx.settings),
