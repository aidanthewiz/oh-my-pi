...
/**
 * Resolve a file against the nearest ancestor that configures a matching LSP.
 * This lets one agent session serve files from nested projects.
 */
export function getFileLspContext(sessionCwd: string, filePath: string): FileLspContext {
	const sessionRoot = path.resolve(sessionCwd);
	const absolutePath = path.resolve(sessionCwd, filePath);
	...
	const config = getConfig(sessionRoot);
	return { cwd: sessionRoot, config, servers: getServersForFile(config, absolutePath) };
}
