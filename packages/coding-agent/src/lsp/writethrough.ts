...
import { getFileLspContext, notifyFileSaved, splitServers, syncFileContent } from "./servers";
...
async function runLspWritethrough(
	...
	const notifyWriteCommitted = async (notifySignal: AbortSignal | undefined = signal) => {
		...
		writeNotified = true;
		try {
			await notifyWorkspaceWatchedFiles(serverCwd, [{ filePath: dst, type: changeType }], notifySignal);
		} catch (error) {
	...
	const context = getFileLspContext(cwd, dst);
	serverCwd = context.cwd;
	...
	const minVersionsPromise = enableDiagnostics
		? captureDiagnosticVersions(serverCwd, servers, 5_000, signal)
		: undefined;
		...
		await untilAborted(operationSignal, async () => {
			if (useCustomFormatter) {
				...
				const [formattedContent, capturedVersions] = await Promise.all([
					formatContent(dst, content, serverCwd, customLinterServers, operationSignal),
					minVersionsPromise,
				]);
				...
				await syncFileContent(dst, finalContent, serverCwd, lspServers, operationSignal, enableDiagnostics);
			} else {
				// 1. Sync original content to LSP servers
				await syncFileContent(dst, content, serverCwd, lspServers, operationSignal);

				// 2. Format in-memory via LSP
				if (enableFormat) {
					const formatted = await formatContent(dst, content, serverCwd, lspServers, operationSignal);
				...
				if (finalContent !== content) {
					await syncFileContent(dst, finalContent, serverCwd, lspServers, operationSignal);
				}
			...
			if (enableDiagnostics) {
				expectedDocumentVersions = await captureOpenFileVersions(dst, serverCwd, lspServers, operationSignal);
			}

			// 5. Notify saved to LSP servers
			await notifyFileSaved(dst, serverCwd, lspServers, operationSignal, !useCustomFormatter || enableDiagnostics);
		});
		synced = true;
	} catch {
		if (timedOut) {
