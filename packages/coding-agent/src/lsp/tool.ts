...
import { configCache, getConfig } from "./config";
import {
	...
	async execute(
		...
		if (action === "diagnostics") {
			...
			for (const target of targets) {
				...
				const { cwd: serverCwd, servers } = getFileLspContext(this.session.cwd, resolved);
				if (servers.length === 0) {
		...
		if (action === "rename_file") {
			...
			const renameContext = getFileLspContext(this.session.cwd, source);
			...
			for (const [serverName, serverConfig] of servers) {
				...
				try {
					client = await getOrCreateClient(serverConfig, renameContext.cwd, undefined, signal);
					if (isProjectAwareLspServer(serverConfig)) {
						await waitForProjectLoaded(client, signal);
					}
				} catch (err) {
		...
		if (action === "capabilities") {
			...
			if (file && file !== "*") {
				...
				const context = getFileLspContext(this.session.cwd, resolved);
				serverCwd = context.cwd;
				serverList = context.servers;
		...
		if (action === "request") {
			...
			if (file && file !== "*") {
				...
				const context = getFileLspContext(this.session.cwd, resolvedTarget);
				serverCwd = context.cwd;
				chosenServer = context.servers[0] ?? null;
				if (!chosenServer) {
					return {
						content: [{ type: "text", text: "No language server found for this file" }],
						details: { action, success: false, request: params },
			...
			try {
				const client = await getOrCreateClient(chosenConfig, serverCwd, undefined, signal);
				if (resolvedTarget) {
		...
		const serverInfo = fileContext?.servers[0] ?? null;
		if (!serverInfo) {
			return {
				content: [{ type: "text", text: "No language server found for this action" }],
				details: { action, success: false },
