...
/** Available models eligible for a role after applying the effective model policy. */
export function roleCandidatePool(role: string, settings: Settings, registry: ModelBrowserRegistry): Model<Api>[] {
	return filterModelsByConfiguredScope(registry.getAvailable("all"), settings).filter(
		getRoleInfo(role, settings).accepts,
	);
}
