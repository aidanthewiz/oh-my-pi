...
import type { BlobDestinationId, BlobDestinationMetadata } from "../blob-broker/destinations";
import { BUILTIN_BLOB_DESTINATIONS } from "../blob-broker/destinations";
...
export const SETTINGS_SCHEMA = {
	...
	"stream.serverUrl": {
		...
		ui: {
			...
			group: "Stream",
			label: "Stream Server",
			description: "Live stream server used by `coreforge stream`; no public server is selected by default",
		},
	},
	...
	"browser.relay": {
		...
		ui: {
			...
			group: "Grep & Browser",
			label: "Browser Relay",
			description: `Drive your own Chrome tabs through the Coreforge browser relay. Install the extension once (\`${CF_COMMAND} browser-relay install\`); the relay server auto-starts when the browser prelude needs it. Takes precedence over Browser CDP URL; set PI_BROWSER_RELAY=0 or PI_BROWSER_RELAY=1 to override.`,
		},
	},
	...
	"browser.relayUrl": {
		...
		ui: {
			...
			description: "Coreforge browser relay endpoint (default http://127.0.0.1:9224).",
		},
	},
