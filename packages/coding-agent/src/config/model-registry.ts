...
export class ModelRegistry {
	...
	constructor(
		...
		// Org-managed overlay lives beside the user models file, so an explicit
		// modelsPath relocates it too. The default remains profile-aware.
	...
	#reloadStaticModels(options?: { force?: boolean; preserveRuntimeDiscovery?: boolean }): void {
		const currentMtime = this.#staticModelsMtimeSignature();
		const staticConfigUnchanged = currentMtime === this.#lastStaticLoadMtime;
		if (!options?.force && staticConfigUnchanged) {
	...
	async #discoverProviderModels(
		...
		this.#providerDiscoveryStates.set(providerId, {
			...
			...(result.updatedAt === undefined ? {} : { fetchedAt: result.updatedAt }),
		...
		try {
			...
			const status: ProviderDiscoveryStatus = authoritativeDiscoveryUnavailable
				...
				: result.source === "cache" ||
					...
					? "cached"
					: result.source === "bundled" && discoveryStale
						? "unavailable"
						: result.source === "bundled"
							? "idle"
							: models.length > 0
			...
			this.#providerDiscoveryStates.set(options.providerId, {
				provider: options.providerId,
				status: "unavailable",
				optional: previous?.optional ?? !options.modelsDev,
				stale: true,
	...
	getAvailableForProviders(providers: ReadonlySet<string>): Model<Api>[] {
		...
		const models = this.#hasFullSnapshot
			? this.#models
			: this.#composeStaticModels(
					new Set(this.#knownStaticProviders().filter(provider => requested.has(provider.toLowerCase()))),
				);
		return models.filter(model => requested.has(model.provider.toLowerCase()) && isModelAvailable(model));
	}
