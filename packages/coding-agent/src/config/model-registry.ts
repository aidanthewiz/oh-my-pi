...
export class ModelRegistry {
	...
	#runtimeProjectionSnapshotValid = false;
	...
	async refresh(strategy: ModelRefreshStrategy = "online-if-uncached"): Promise<void> {
		if (this.#runtimeProjectionSnapshotValid) {
			this.#runtimeProjectionSnapshotValid = false;
	...
	#clearInternedProvider(provider: string): void {
		const prefix = `${provider}\u0000`;
		for (const key of this.#internedStaticModels.keys()) {
			if (key.startsWith(prefix)) this.#internedStaticModels.delete(key);
		}
	}
	...
	#modelsForProviderLookup(provider: string): Model<Api>[] {
		if (this.#hasFullSnapshot || this.#runtimeProjectionSnapshotValid) return this.#models;
	...
	hasProvider(providerId: string): boolean {
		const providerModels =
			this.#hasFullSnapshot || this.#runtimeProjectionSnapshotValid
				? this.#models
	...
	registerProvider(providerName: string, config: ProviderConfigInput, sourceId?: string): void {
		...
		if (config.models && config.models.length > 0) {
			...
			this.#models = this.#applyRuntimeModelModifiers(this.#unprojectedModels);
			this.#hasFullSnapshot = false;
			this.#runtimeProjectionSnapshotValid = true;
			this.#providerLookupSnapshots.clear();
			return;
		}
